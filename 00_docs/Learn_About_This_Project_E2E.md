...
- Studio API(백엔드): `backend/apps/core/views.py`
  ...
  - `studio_llm`: fal(OpenRouter router) 기반 LLM
  - `studio_image`: fal 이미지 생성
  - `studio_tts`: fal MiniMax TTS (`backend/apps/ai/fal_client.py::tts_minimax`)
...
- Studio 워크플로우 이해
  ...
  - 어떤 API를 언제 호출하는지(trending/context/benchmark/llm/image/tts)
- 확장 과제(연습)
